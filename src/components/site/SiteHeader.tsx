...
export function SiteHeader({ onOpenEnquire }: { onOpenEnquire?: () => void } = {}) {
  ...
  const isItemActive = (item: NavItem) => {
    if (item.to === "/") {
      return onHome && (activeSection === "home" || !location.hash) && activeSection !== "contact";
    }
    if (item.to) {
  ...
  const renderLink = (item: NavItem, className: string, onClick?: () => void) => {
    ...
    if (item.to) {
      return (
        <Link to={item.to} className={className} onClick={onClick} onMouseEnter={() => setHovered(item.label)}>
          {navLabel(item)}
        </Link>
      );
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <img src={getImgSrc(finprovMark)} alt="" className="h-full w-full object-cover" />
          </motion.div>
