...
export function RAGChatbot() {
  ...
  const handleSend = (textToSend?: string) => {
    ...
    setTimeout(() => {
      const ragResult = searchRAG(query);
      const botMsg: RAGMessage = {
        id: `bot-${Date.now()}`,
        sender: "bot",
        text: ragResult.response,
        recommendedCourses: ragResult.recommendedCourses,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      };
  ...
  return (
    <>
      ...
      <motion.button
        ...
        aria-label="Toggle Fin Bot"
        className="fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-40 flex h-11 items-center gap-2 rounded-full bg-navy px-4 py-2 text-white shadow-xl ring-2 ring-gold/40 transition-transform hover:scale-105"
      ...
      >
        ...
        <span className="text-[11px] font-extrabold tracking-wide uppercase">Fin Bot</span>
      </motion.button>

      {/* RAG Chat Drawer Modal */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
