...
import { hiringPartners, placementStats, testimonials } from "@/data/testimonials";

export const Route = createFileRoute("/placements")({
  head: () => ({
    meta: [
      { title: "Placements — Finprov Career Outcomes" },
      { name: "description", content: "4,500+ learners placed, 300+ hiring companies, 95% placement record. See Finprov's placement highlights and success stories." },
    ],
  }),
...
const topPartners = hiringPartners.slice(0, Math.ceil(hiringPartners.length / 2));
const bottomPartners = hiringPartners.slice(Math.ceil(hiringPartners.length / 2));
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden bg-navy py-20 text-white">
        ...
        <div className={`${container} relative text-center`}>
          ...
          >
            Real outcomes from learners who trained on the tools employers actually use.
          </motion.p>
          ...
          <StaggerGrid className="glass-dark gloss-soft mx-auto mt-14 grid max-w-3xl grid-cols-2 gap-8 rounded-3xl p-8 md:grid-cols-4">
            <StaggerItem>
              <Stat value={<CountUp end={placementStats.learnersPlaced} />} label="Learners Placed" />
            </StaggerItem>
            <StaggerItem>
              <Stat value={<CountUp end={placementStats.uniqueRolesExplored} suffix="+" />} label="Unique Roles Explored" />
            </StaggerItem>
            <StaggerItem>
              <Stat value={<CountUp end={placementStats.interviewsConducted} suffix="+" />} label="Interviews Conducted" />
            </StaggerItem>
            <StaggerItem>
              <Stat value={<CountUp end={placementStats.companiesHired} suffix="+" />} label="Companies Hired" />
            </StaggerItem>
          </StaggerGrid>
        </div>
      </section>
      ...
      <section className="py-24">
        <div className={`${container} grid gap-16 lg:grid-cols-2 lg:items-center`}>
          <Reveal>
            ...
            <p className="mt-4 max-w-lg text-base leading-relaxed text-text-body">
              Our supportive environment combines experienced instructors with hands-on, tool-led learning. We back
              every cohort with 100% placement assistance and a {placementStats.placementRecord}% placement record.
            </p>
