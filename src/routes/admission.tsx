...
export const Route = createFileRoute("/admission")({
  validateSearch: (search: Record<string, unknown>): AdmissionSearch => ({
...
export function OnlineAdmission() {
  let searchParamCourse = "";
  try {
    const search = Route.useSearch();
    searchParamCourse = search?.course || "";
  } catch (err) {
    searchParamCourse = "";
  }
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const c = urlParams.get("course") || searchParamCourse;
      if (c && courses.some((crs) => crs.slug === c)) {
        setFormData((prev) => ({ ...prev, selectedCourseSlug: c }));
      }
    }
  }, [searchParamCourse]);
      ...
      <section className="relative overflow-hidden bg-navy py-14 text-white">
        ...
        <div className={`${container} relative text-center`}>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-gold/15 px-3.5 py-1 text-xs font-bold text-gold ring-1 ring-gold/30">
            <Sparkles className="h-3.5 w-3.5" /> Official Online Admission Portal
          </span>
          <h1 className="mt-4 text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl">
            Complete Your Finprov Admission Online
          </h1>
