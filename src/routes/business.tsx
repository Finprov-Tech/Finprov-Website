...
export const Route = createFileRoute("/business")({
  head: () => ({
    meta: [
      { title: "Finprov Business — Corporate Training Solutions" },
      { name: "description", content: "Custom corporate training, workforce upskilling, and hire-train-deploy solutions from Finprov." },
    ],
  }),
  component: Business,
});
...
function Business() {
  const isProgramPage = useRouterState({
    select: (state) => state.location.pathname.startsWith("/business/"),
  });
  const [selectedAreaTitle, setSelectedAreaTitle] = useState<string>(businessProgramAreas[0].title);
  const [expandedAreas, setExpandedAreas] = useState<Set<string>>(new Set());

  if (isProgramPage) return <Outlet />;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className={`${container} relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center`}>
          <Reveal>
            ...
            <h1 className="mt-3 max-w-3xl text-3xl font-extrabold leading-tight tracking-tight sm:text-4xl lg:text-5xl">
              Upskill Your Team, <span className="text-gold">Strengthen Your Business!</span>
            </h1>
                  ...
                  <div>
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                      {visiblePrograms.map((program) => {
                        ...
                        return hasOriginalBusinessProgramDetail(program) ? <Link key={program} to="/business/$program" params={{ program: businessProgramSlug(program) }} className={className}>{cardContent}</Link> : <a key={program} href={site.whatsappHref} target="_blank" rel="noreferrer" className={className}>{cardContent}</a>;
                      })}
                    </div>
                  </div>
      ...
      <section className="section-ambient py-24">
        <div className={container}>
          <Reveal><p className="text-sm font-bold uppercase tracking-[0.18em] text-cta">Business solutions</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-navy sm:text-5xl">How Businesses Grow With Finprov Training</h2><p className="mt-4 max-w-2xl text-text-body">Build a future-ready workforce with learning solutions shaped around your business goals. We bring the right skills, the right talent, and the right training to drive organisational growth.</p></Reveal>
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {solutions.map(([title, description, Icon], index) => (
              <Reveal key={title} delay={index * 0.06}>
                <div className="glass gloss-soft h-full rounded-3xl p-7"><Icon className="h-8 w-8 text-cta" /><h3 className="mt-5 text-xl font-bold text-navy">{title}</h3><p className="mt-3 leading-relaxed text-text-body">{description}</p></div>
              </Reveal>
            ))}
          </div>
          <Reveal>
            <div className="mt-12 rounded-3xl bg-gradient-to-r from-cta via-teal to-cta p-8 text-center text-white shadow-xl shadow-cta/20 sm:p-12">
              <h2 className="text-3xl font-bold">Connect With Our Experts</h2>
              <p className="mx-auto mt-3 max-w-2xl text-white/80">Find the right learning and hiring solutions with support from our expert team.</p>
              <Link to="/" hash="contact" className="btn-gloss mt-7 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3.5 font-bold text-navy">Talk to Finprov <ArrowUpRight className="h-4 w-4" /></Link>
            </div>
          </Reveal>
        </div>
      </section>
