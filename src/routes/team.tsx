...
function ScrollMemberCard({
  ...
  return (
    ...
    >
      {member.photo ? (
        <img src={getImgSrc(member.photo)} alt="" className="h-full w-full object-cover object-top" />
      ) : (
...
export function TeamPage() {
  ...
  return (
    <div className="min-h-screen bg-[#06101f] font-sans text-white antialiased selection:bg-[#ffb703] selection:text-[#06101f]">
      <SiteHeader onOpenEnquire={() => setEnquireOpen(true)} />
        ...
        <section className="relative overflow-hidden bg-navy py-20 text-white sm:py-28 border-t border-b border-white/10">
          ...
          <div className={`${container} relative`}>
            <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
              <Reveal>
                <p className="text-[11px] font-black uppercase tracking-[.28em] text-gold">Learn from people who do the work</p>
                <h2 className="mt-4 max-w-4xl text-4xl font-black leading-[.95] tracking-[-.05em] sm:text-6xl text-white">
                  Your next mentor is <span className="text-gold">already here.</span>
                </h2>
              </Reveal>
              <div className="flex flex-wrap gap-3">
                <Link to="/courses" className="inline-flex items-center gap-2.5 rounded-full bg-gold px-7 py-3.5 text-sm font-black text-navy shadow-xl transition-all hover:scale-105 hover:bg-gold-light">
                  Explore programs <ArrowUpRight className="h-4 w-4 stroke-[2.5]" />
                </Link>
      ...
      <SiteFooter onOpenEnquire={() => setEnquireOpen(true)} />
