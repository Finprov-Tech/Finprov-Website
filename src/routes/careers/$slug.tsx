...
import { ArrowLeft, Briefcase, Building2, CheckCircle2, Clock, Mail, MapPin, Phone, Send, Share2, User } from "lucide-react";
...
export const Route = createFileRoute("/careers/$slug")({
  head: ({ params }) => {
    ...
    return {
      meta: [
        { title: job ? `${job.title} — Finprov Careers` : "Job Opening — Finprov Careers" },
        { name: "description", content: job ? `Apply for ${job.title} at Finprov Learning in ${job.location}. View responsibilities, qualifications, and application process.` : "Job opening details at Finprov." },
      ],
    };
  },
  component: JobDetail,
    ...
    return (
      <div className="min-h-screen bg-background text-foreground">
        ...
        <div className={`${container} py-32 text-center`}>
          <h1 className="text-3xl font-extrabold text-navy">Position Not Found</h1>
          <p className="mt-2 text-navy/70">The job opening you are looking for may have been filled or updated.</p>
          <Link
            to="/careers"
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-navy px-6 py-3 text-sm font-semibold text-white hover:bg-navy/90"
          >
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="py-16">
        <div className={`${container} grid gap-12 lg:grid-cols-[1fr_360px]`}>
          ...
          {/* Left Column: Full Word-to-Word Job Description Content */}
